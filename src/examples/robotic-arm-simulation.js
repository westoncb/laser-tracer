...
function drawElbow(pen, angle = 0, highlight = false) {
  ...
  for (let i = 0; i < segments; i += 2) {
    ...
    pen.polyline(
      [
        {
          x: startRadius * Math.cos(theta),
          y: startRadius * Math.sin(theta),
          z: 0,
        },
        {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    const radius =
      startRadius * (1 - z / armLength) + endRadius * (z / armLength);
...
function drawWrist(
  ...
  pen.polyline(
    [
      {
        x: -gripperBaseWidth / 2 - gripWidth - gripperThickness,
        y: 0,
        z: gripperLength + 0.5,
      },
      { x: -gripperBaseWidth / 2 - gripWidth, y: 0, z: gripperLength + 0.5 },
      {
        x: -gripperBaseWidth / 2 - gripWidth,
        y: 0,
        z: gripperLength + 0.5 - gripperThickness,
      },
    ],
    false,
  );
...
function drawScanPattern(pen, target, time) {
  ...
  if (target.type === "cube") {
    ...
    if (scanPoints.length > 0) {
      pen.moveTo(
        scanPoints[scanPoints.length - 1].x,
  ...
  switch (testPhase) {
    case 0: // Initialization
      ...
      sensorReadings = [
        {
          label: "VOLTAGE",
          value: (12 + Math.sin(time * 2)).toFixed(2) + "V",
          color: { r: 0, g: 1, b: 0 },
        },
        {
          label: "TEMP",
          value: (20 + phaseTime * 2).toFixed(1) + "°C",
          color: { r: 0, g: 1, b: 0 },
        },
        {
          label: "CPU",
          value: (50 + phaseTime * 5).toFixed(0) + "%",
          color: { r: 0, g: 1, b: 0 },
        },
      ];
      ...
      sensorReadings = [
        {
          label: "BASE TORQUE",
          value:
            (5 + Math.abs(Math.cos(phaseTime * 0.5) * 3)).toFixed(2) + "Nm",
          color: { r: 0, g: 1, b: 0 },
        },
        {
          label: "ROTATION",
          value: baseRotation.toFixed(1) + "°",
          color: { r: 0, g: 1, b: 0 },
        },
      ...
      sensorReadings = [
        {
          ...
          value: (20 + Math.abs(Math.sin(t) * 10)).toFixed(1) + "%",
          color: { r: 0, g: 1, b: 0 },
        },
        {
          label: "ELBOW STRAIN",
          value: (15 + Math.abs(Math.sin(t + 1) * 10)).toFixed(1) + "%",
          color: { r: 0, g: 1, b: 0 },
        },
        {
          label: "WRIST STRAIN",
          value: (10 + Math.abs(Math.sin(t + 2) * 10)).toFixed(1) + "%",
          color: { r: 0, g: 1, b: 0 },
        },
      ];
    ...
    case 3: // Grip strength test
      ...
      sensorReadings = [
        {
          label: "GRIP FORCE",
          value: (50 - gripperWidth * 25).toFixed(1) + "N",
          color: { r: 0, g: 1, b: 0 },
        },
        {
          label: "MOTOR TEMP",
          value: (25 + (1 - gripperWidth) * 10).toFixed(1) + "°C",
          color: { r: 0, g: 1, b: 0 },
        },
        {
          label: "GRIP WIDTH",
          value: gripperWidth.toFixed(2) + " units",
          color: { r: 0, g: 1, b: 0 },
        },
      ];
    ...
    case 4: // Tool operation test
      ...
      diagnosticText = [
        ...
        "POWER OUTPUT: " +
          (toolExtension
            ? (50 + Math.sin(time * 5) * 5).toFixed(1) + "W"
            : "0.0W"),
      ];
      ...
      sensorReadings = [
        {
          label: "TOOL TEMP",
          value: (30 + (toolExtension ? 20 : 0)).toFixed(1) + "°C",
          color: toolExtension ? { r: 1, g: 0.5, b: 0 } : { r: 0, g: 1, b: 0 },
        },
        {
          label: "POWER DRAW",
          value: (toolExtension ? 15 : 5).toFixed(1) + "W",
          color: toolExtension ? { r: 1, g: 0.5, b: 0 } : { r: 0, g: 1, b: 0 },
        },
        {
          label: "FOCAL DIST",
    ...
    case 5: // Object interaction test
      ...
      sensorReadings = [
        {
          label: "PROXIMITY",
          value: (20 + Math.sin(phaseTime * 0.4) * 5).toFixed(1) + "mm",
          color: { r: 0, g: 1, b: 0 },
        },
        {
          label: "HARDNESS",
          value: (75 + Math.sin(phaseTime) * 5).toFixed(1),
          color: { r: 0, g: 1, b: 0 },
        },
        {
          label: "SCAN COMP",
          value: Math.min(100, phaseTime * 10).toFixed(0) + "%",
