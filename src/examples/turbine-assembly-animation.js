...
const MATERIALS = {
  ...
  HIGHLIGHT: {
    name: "HIGHLIGHT",
    color: { r: 1, g: .6, b: 1 },  // Bright white
    dotSize: 4,
    traceGap: 0.1,
    fuzzSize: 0.05,
    fuzz: 3,
...
function applyPatternToSurface(pen, material, points, spacing, angle = 0) {
  ...
  if (material.pattern === "hatched") {
    ...
    for (let i = 0; i <= lines; i++) {
      ...
      const thirdAxis = 'xyz'.replace(primary.axis, '').replace(secondary.axis, '');
...
function createTurbineBlade(pen, angle, extension = 0, showHighlight = false) {
  ...
  if (showHighlight && partLabelsVisible) {
    pen.colorRGB(1, 1, 1);
    draw.text("TURBINE BLADE", { x: rootDist + bladeLength/2, y: bladeWidth + 3, z: 0 }, 1);
    draw.text("Ti-6Al-4V ALLOY", { x: rootDist + bladeLength/2, y: bladeWidth + 1.5, z: 0 }, 0.8);
  }
...
function drawTurbineHousing(pen, offset = 0, showHighlight = false, showCrossSection = false) {
  pen.push();
  ...
  for (let i = 0; i < housing.segments; i += 2) {
    ...
    if (showCrossSection && (Math.abs(angle - 0) < 0.1 || Math.abs(angle - Math.PI) < 0.1)) {
      continue;
    }
  ...
  for (let side = -1; side <= 1; side += 2) {
    ...
    for (let i = 0; i < housing.segments; i += 3) {
      ...
      pen.polyline([
        ...
        { x: Math.cos(angle) * flangeRadius, y: Math.sin(angle) * flangeRadius, z: z }
      ], false);
    }
  }
  ...
  if (showHighlight && partLabelsVisible) {
    pen.colorRGB(1, 1, 1);
    draw.text("TURBINE HOUSING", { x: 0, y: housing.midRadius + 4, z: 0 }, 1);
    draw.text("INCONEL 718", { x: 0, y: housing.midRadius + 2.5, z: 0 }, 0.8);
  }
...
function drawAssemblyArrow(pen, from, to, time, phaseStart, phaseDuration, arrowWidth = 1) {
  pen.push();
  ...
  pen.dotSize(4).traceGap(0.1).fuzz(2, 0.05).residue(0.5);
...
function drawFloatingLabels(pen, time) {
  ...
  if (animPhase === 1 || animPhase === 7) {
    ...
    draw.text("Ø" + (midRadius * 2).toFixed(2), { x: midRadius * 0.6, y: midRadius * 0.6, z: 5 }, 1);
  ...
  if ((animPhase === 0 || animPhase === 4 || animPhase === 5 || animPhase === 8) && 
      Math.sin(time * 5) > 0) {
    statusText = "";
  }
  ...
  switch(animPhase) {
    ...
    case 3: // Mount blades
      ...
      if (arrowProgress < 0.9) {
        pen.push();
        pen.colorRGB(1, 1, 0.5);
        pen.dotSize(3).traceGap(0.1).fuzz(2, 0.05);
...
function program(pen, draw, time) {
  ...
  if (crossSectionView) {
    ...
    const bladeCount = 9;
    const bladeExtension = animPhase === 6 ? 5 * easeInOut((time % (ANIM_DURATION / 9)) / (ANIM_DURATION / 9)) : 0;
    ...
    for (let i = 0; i < bladeCount; i++) {
      ...
      if (angle % 360 > 180 && angle % 360 < 360) {
        createTurbineBlade(pen, angle, bladeExtension, highlightPart === "blades");
      }
    }
  } else {
