...
function createCoordinateAxes(length) {
  return {
    x: [{ x: 0, y: 0, z: 0 }, { x: length, y: 0, z: 0 }],
    y: [{ x: 0, y: 0, z: 0 }, { x: 0, y: length, z: 0 }],
    z: [{ x: 0, y: 0, z: 0 }, { x: 0, y: 0, z: length }]
  };
}
...
function drawMaterializingCube(pen, time, startTime, duration) {
  ...
  if (progress >= 0.4 && progress < 0.7) {
    ...
    for (let i = 0; i < Math.ceil(frontGrid.length * gridProgress); i++) {
      if (i < frontGrid.length) {
        ...
        const adjustedLine = frontGrid[i].map(pt => ({ ...pt, z: -CUBE_SIZE }));
  ...
  if (progress >= 0.6) {
    ...
    if (finalProgress > 0.8) {
      ...
      pen.dotSize(2)
      pen.moveTo(0, 0, -CUBE_SIZE)
        .text("FACE 1", .7);

      pen.moveTo(0, -CUBE_SIZE, 0)
        .text("FACE 2", .7);
    ...
    if (finalProgress > 0.9) {
      ...
      pen.moveTo(xDimension.labelPos.x, xDimension.labelPos.y, xDimension.labelPos.z)
      ...
      pen.moveTo(zDimension.labelPos.x, zDimension.labelPos.y, zDimension.labelPos.z)
...
function drawCalibrationScan(pen, time, startTime, duration) {
  ...
  pen.moveTo(textPos.x, textPos.y, textPos.z)
    .traceGap(0.1)
    .dotSize(6)
    .colorHex(0xff0000)
    .text(scanText, 2);
  ...
  for (let i = 0; i < metadataLines.length; i++) {
    if (progress >= metadataLines[i].threshold) {
      ...
      if (i === metadataLines.length - 1 && progress > 0.95) {
        ...
        pen.colorRGB(0, 1, 0.4 + pulseEffect * 0.6)
          .dotSize(STYLES.secondary.dotSize * (1 + pulseEffect * 0.3)).traceGap(0.1)
          .fuzz(STYLES.secondary.fuzz * (1 + pulseEffect * 0.5), STYLES.secondary.fuzzSize);
...
function program(pen, d, time) {
  // First time setup
  if (firstFrame) {
    setBGColor(0x00000A); // Almost black background
    firstFrame = false;
  }
  ...
  if (time < phases.boot.start + phases.boot.duration) {
    ...
    const bootProgress = (time - phases.materialize.start) / phases.materialize.duration;
    ...
    const elevation = (baseElevation + Math.sin(time * 0.3) * 2) * (Math.PI / 180);
  ...
  if (time >= phases.materialize.start - 2 && time < phases.calibrate.start) {
    drawMaterializingCube(pen, time, phases.materialize.start, phases.materialize.duration);
  }
  
  // Phase 3: Calibration
  if (time >= phases.calibrate.start) {
    if (time <= phases.calibrate.start + BOOT_DURATION) {
      drawCalibrationScan(pen, time, phases.calibrate.start, phases.calibrate.duration);
    }
    drawCalibrationMetadata(pen, time, phases.calibrate.start, phases.calibrate.duration);
    drawCoordinateReadouts(pen, time, phases.calibrate.start, phases.calibrate.duration);
    drawMaterializingCube(pen, time, phases.materialize.start, phases.materialize.duration);
  }
