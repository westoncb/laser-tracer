...
function platonicEdges(type, r) {
  ...
  if (type === "cube") {
    for (let xi of [-1, 1])
      for (let yi of [-1, 1])
        for (let zi of [-1, 1])
          if (xi === -1) edges.push([{ x: xi * r, y: yi * r, z: zi * r }, { x: yi * r, y: xi * r, z: zi * r }]);
  } else if (type === "octa") {
    const v = [
      { x: 0, y: 0, z: r },
      { x: 0, y: 0, z: -r },
    ...
    const faces = [
      [0, 8, 9, 4,14], [0,14,12,2,16], [0,16,18,1,8],
      [1,18,19,5,9], [1,9,8,0,16], [2,12,13,3,10],
      [2,10,11,6,16], [3,13,15,7,11], [3,11,10,2,12],
      [4,9,5,19,17], [4,17,12,0,14], [5,19,18,1,9],
      [6,11,7,15,17], [6,17,14,0,16], [7,15,13,3,11],
      [8,1,9,5,4], [10,2,12,17,6], [13,2,14,4,15],
      [18,16,6,17,19], [19,17,15,13,18],
    ];
...
function program(pen, draw, time) {
  ...
  // orbitCamera({ x: 0, y: 0, z: 0 }, 80, time * 6, 25);
  ...
  //// 1 · Φ‑vortex
  pen.push();
  pen.dotSize(3.8).fuzz(24, 0.5).traceGap(0.18).residue(1.4);
  ...
  //// 2 · Lorenz storm
  pen.push();
  pen.dotSize(1.8).fuzz(0).traceGap(0.22).residue(0.7);
