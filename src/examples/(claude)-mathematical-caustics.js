...
function program(pen, draw, time) {
  ...
  function createLens(p, time) {
    ...
    const detail = Math.sin(p.x * PHI + p.y * PSI + p.z * SQRT5 + time) * 0.5 + 0.5;
  ...
  function vectorField(p, time) {
    ...
    const attention = p.x * Math.sin(time * PHI) + 
  ...
  pen.yaw(Math.sin(t * 0.3) * 10)
     .pitch(Math.cos(t * 0.2) * 8);
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      for (let j = 0; j < steps; j++) {
        ...
        if (j % 3 === 0) {
          ...
          const attention = p.x * newLens.chromatic.x + p.y * newLens.chromatic.y + p.z * newLens.chromatic.z;
          const newHue = (hue + attention * 20) % 360;
          pen.colorHSV(newHue, saturation, value);
        }
      }
  ...
  function drawMathematicalSurfaces(time) {
    ...
    for (let s = 0; s < loci; s++) {
      ...
      for (let i = 0; i < segments; i++) {
        ...
        const r = 1.0 + Math.sin(theta * lens.chromatic.x + time) * 0.5 * lens.aperture;
      ...
      for (let i = 0; i < segments; i++) {
        const theta = i / segments * TAU;
        const r = 3.0 + Math.sin(theta * lens.chromatic.y + time * 1.3) * 1.5 * lens.aperture;
      ...
      pen.moveTo(center.x, center.y, center.z)
         .yaw(time * 30 + s * 90)
  ...
  function highlightIntersections(time) {
    ...
    for (let i = 0; i < points; i++) {
      ...
      pen.push()
         .colorHSV(
           Math.sin(lens.focal * PHI) * 60 + 30, // Golden hues
