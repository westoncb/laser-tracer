...
//// ── tweakables ────────────────────────────────────────────────
const HUB_R   = 8,   HUB_H   = 8,   SEG = 24;
const SHAFT_R = 2,   SHAFT_L = 18;
const BLADE_N = 9,   BLADE_L = 10,  BLADE_W = 3;
...
function mat(p, m) { return p.colorRGB(m.r,m.g,m.b).dotSize(m.size)
                       .traceGap(m.gap).fuzz(2,0.1).residue(0.8); }

//// ── geometry builders ────────────────────────────────────────
function hub(p){
  p.push(); mat(p,Ti);
  for(const z of [-HUB_H/2,HUB_H/2]){
    const ring=[]; for(let i=0;i<=SEG;i++){
      const a=i/SEG*2*Math.PI;
      ring.push({x:Math.cos(a)*HUB_R,y:Math.sin(a)*HUB_R,z});
    } p.polyline(ring,true);
  }
  for(let i=0;i<SEG;i+=3){
    const a=i/SEG*2*Math.PI, x=Math.cos(a)*HUB_R, y=Math.sin(a)*HUB_R;
    p.polyline([{x,y,z:-HUB_H/2},{x,y,z:HUB_H/2}],false);
  }
  p.pop();
}
...
function shaft(p){
  ...
  for(let i=0;i<SEG;i+=4){
    const a=i/SEG*2*Math.PI, x=Math.cos(a)*SHAFT_R, y=Math.sin(a)*SHAFT_R;
    p.polyline([{x,y,z:-SHAFT_L/2},{x,y,z:SHAFT_L/2}],false);
  }
  p.pop();
}
...
function blade(p){
  p.push(); mat(p,Ti);
  const pts=[], N=12, ROOT=HUB_R+1;            // clear the hub wall
  for(let i=0;i<=N;i++){                       // upper surface
    const t=i/N;
    pts.push({x:ROOT+t*BLADE_L,
...
function housing(p){
  ...
  const rings=[];
  for(let i=0;i<=5;i++){
    const t=i/5, z=HOUSING.z0+t*(HOUSING.z1-HOUSING.z0);
    const r=t<0.5 ?               // bulge interpolation
            HOUSING.r0*(1-2*t)+HOUSING.rMid*(2*t) :
            HOUSING.rMid*(1-2*(t-0.5))+HOUSING.r1*(2*(t-0.5));
    const pts=[]; for(let j=0;j<=SEG;j++){
      const a=j/SEG*2*Math.PI;
      pts.push({x:Math.cos(a)*r,y:Math.sin(a)*r,z});
    }
    p.polyline(pts,true); rings.push(pts);
  }
  for(let i=0;i<SEG;i+=3){                       // longitudinal seams
    p.polyline(rings.map(r=>r[i]),false);
  }
  p.pop();
}
...
function grid(p){
  ...
  const G=40,S=5;
  for(let x=-G;x<=G;x+=S)
    p.polyline([{x,y:-25,z:-G},{x,y:-25,z:G}],false);
  for(let z=-G;z<=G;z+=S)
    p.polyline([{x:-G,y:-25,z},{x:G,y:-25,z}],false);
  p.pop();
}
...
function program(pen,time){
  if(first){ setBGColor(0x000010); first=false; }
  ...
  // hub (shifted)
  pen.push(); pen.moveBy(0,0,hubZ); hub(pen); pen.pop();

  // blades disc at bladeZ, spinning in local roll
  pen.push(); pen.moveBy(0,0,bladeZ);
  for(let i=0;i<BLADE_N;i++){
    pen.push(); pen.roll(i/BLADE_N*360); blade(pen); pen.pop();
  }
  pen.pop();
