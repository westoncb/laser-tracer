...
function drawDimensions(pen) {
  ...
  draw.text("Ø" + (radius * 2).toFixed(2), { x: radius * 0.85, y: radius * 0.85, z: 0 }, 0.8);
...
function drawInfoPanel(pen, time) {
  ...
  // Panel title
  draw.text("TURBINE ASSEMBLY", { x: panelX + 2, y: panelY - 2, z: panelZ }, 1.2);
...
function program(pen, draw, time) {
  ...
  for (let i = 0; i < materials.length; i++) {
    ...
    // Label
    draw.text(mat.name, { x: legendX + 6, y: legendY - 2 - i * 1.5, z: legendZ }, 0.9);
  }
