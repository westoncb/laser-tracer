...
function applyReflections(point, depth, time) {
  ...
  for (let i = 0; i < 4; i++) {
    // 4 mirrors in the Coxeter group [3,3,5]
...
function program(pen, draw, time) {
  ...
  for (let i = 0; i < POINT_COUNT; i++) {
    ...
    if (i % 10 === 0) {
      // Only apply reflections to some points for performance
  ...
  for (let i = 0; i < visiblePoints.length; i++) {
    ...
    for (let j = i + 1; j < visiblePoints.length; j++) {
      ...
      if (dist < connectionDistance) {
        ...
        const hue =
          (p1.color.hue * blendFactor + p2.color.hue * (1 - blendFactor)) % 1;
        const sat =
          p1.color.sat * blendFactor + p2.color.sat * (1 - blendFactor);
        const val =
          p1.color.val * blendFactor + p2.color.val * (1 - blendFactor);
