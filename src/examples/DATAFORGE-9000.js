...
function applyMaterial(pen, material) {
  pen.colorRGB(material.color.r, material.color.g, material.color.b)
...
function drawTerminalCase(pen) {
  ...
  pen.polyline([
    { x: -dimensions.width/2, y: 0, z: -dimensions.depth/2 },
    { x: -dimensions.width/2, y: dimensions.height, z: -dimensions.depth/2 + 5 },
    { x: dimensions.width/2, y: dimensions.height, z: -dimensions.depth/2 + 5 },
    { x: dimensions.width/2, y: 0, z: -dimensions.depth/2 },
    { x: -dimensions.width/2, y: 0, z: -dimensions.depth/2 }
  ], true);
  ...
  draw.text("DATAFORGE-9000", { x: -15, y: 5, z: dimensions.depth/2 - 4.5 }, 1.5);
  draw.text("QUANTUM DYNAMICS CORP", { x: -12, y: 3, z: dimensions.depth/2 - 4.5 }, 0.9);
  
  // Model details
  applyMaterial(pen, MATERIALS.INDICATOR);
  draw.text("SN: QD-9284-B", { x: dimensions.width/2 - 10, y: 1, z: dimensions.depth/2 - 1 }, 0.7);
...
function drawScreenHousing(pen, time) {
  ...
  // Draw outer case
  const casePoints = [
    { x: -dims.width/2 - dims.bezelThickness, y: -dims.height/2 - dims.bezelThickness, z: 0 },
    { x: dims.width/2 + dims.bezelThickness, y: -dims.height/2 - dims.bezelThickness, z: 0 },
    { x: dims.width/2 + dims.bezelThickness, y: dims.height/2 + dims.bezelThickness, z: 0 },
    { x: -dims.width/2 - dims.bezelThickness, y: dims.height/2 + dims.bezelThickness, z: 0 },
    { x: -dims.width/2 - dims.bezelThickness, y: -dims.height/2 - dims.bezelThickness, z: 0 }
  ...
  // Draw back panel
  pen.polyline([
    { x: -dims.width/2 - dims.bezelThickness, y: -dims.height/2 - dims.bezelThickness, z: -dims.depth },
    { x: dims.width/2 + dims.bezelThickness, y: -dims.height/2 - dims.bezelThickness, z: -dims.depth },
    { x: dims.width/2 + dims.bezelThickness, y: dims.height/2 + dims.bezelThickness, z: -dims.depth },
    { x: -dims.width/2 - dims.bezelThickness, y: dims.height/2 + dims.bezelThickness, z: -dims.depth },
    { x: -dims.width/2 - dims.bezelThickness, y: -dims.height/2 - dims.bezelThickness, z: -dims.depth }
  ...
  for (let x = -dims.width/3; x <= dims.width/3; x += 4) {
    pen.polyline([
      { x: x - 1.5, y: dims.height/2 + dims.bezelThickness, z: -dims.depth/4 },
      { x: x + 1.5, y: dims.height/2 + dims.bezelThickness, z: -dims.depth/4 },
      { x: x + 1.5, y: dims.height/2 + dims.bezelThickness, z: -dims.depth/4 - 3 },
      { x: x - 1.5, y: dims.height/2 + dims.bezelThickness, z: -dims.depth/4 - 3 },
      { x: x - 1.5, y: dims.height/2 + dims.bezelThickness, z: -dims.depth/4 }
    ], true);
  }
  ...
  // Screen status LEDs
  applyMaterial(pen, powerState === "on" ? MATERIALS.INDICATOR : MATERIALS.DARK);
  ...
  for (let i = 0; i < 3; i++) {
    ...
    pen.polyline([
      ...
      { x: knobX + Math.cos(radAngle) * knobSize, y: knobY + Math.sin(radAngle) * knobSize, z: knobZ }
    ], false);
...
function drawScreenContent(pen, time) {
  ...
  if (bootSequence < 1.0) {
    ...
    if (pattern === 0) {
      ...
      // Pattern info
      applyMaterial(pen, MATERIALS.INDICATOR);
      draw.text("LISSAJOUS PATTERN", { x: -10, y: -dims.height/2 + 2, z: 0.2 }, 1);
      draw.text(`A=${a.toFixed(2)} B=${b.toFixed(2)}`, { x: -8, y: -dims.height/2 + 4, z: 0.2 }, 0.8);
      ...
      // Pattern info
      applyMaterial(pen, MATERIALS.INDICATOR);
      draw.text("WAVEFORM ANALYSIS", { x: -10, y: -dims.height/2 + 2, z: 0.2 }, 1);
      const freq = (0.3 + 0.1 * Math.sin(time * 0.2)) * 10;
      draw.text(`FREQ=${freq.toFixed(2)}Hz`, { x: -8, y: -dims.height/2 + 4, z: 0.2 }, 0.8);
      ...
      draw.text(`TURNS=${spiralTurns.toFixed(1)}`, { x: -8, y: -dims.height/2 + 4, z: 0.2 }, 0.8);
    }
    ...
    draw.text(`SYS: ${timeString}`, { x: dims.width/2 - 10, y: dims.height/2 - 2, z: 0.2 }, 0.8);
    ...
    for (let i = 0; i < statusIndicators.length; i++) {
      draw.text(statusIndicators[i], { x: dims.width/2 - 10, y: dims.height/2 - 4 - i * 1.5, z: 0.2 }, 0.7);
    }
    ...
    applyMaterial(pen, MATERIALS.PHOSPHOR);
    draw.text("CMD>", { x: -dims.width/2 + 2, y: -dims.height/2 + 2, z: 0.2 }, 0.8);
...
function drawControlPanel(pen, time) {
  ...
  for (const button of controlButtons) {
    ...
    // Button label
    draw.text(button.label, { x: button.x - btnWidth/2 + 0.5, y: button.y - 0.5, z: 0.2 }, 0.9);
  }
  ...
  pen.polyline([
    ...
    { x: dialX + Math.cos(posAngle) * dialRadius, y: dialY + Math.sin(posAngle) * dialRadius, z: 0.2 }
  ], false);
  ...
  for (let i = 0; i < ledCount; i++) {
    ...
    if (ledOn) {
      applyMaterial(pen, i % 3 === 0 ? MATERIALS.INDICATOR : 
                         i % 3 === 1 ? MATERIALS.WARNING : 
                         MATERIALS.PHOSPHOR);
    } else {
      applyMaterial(pen, MATERIALS.DARK);
    }
  ...
  applyMaterial(pen, MATERIALS.CONTROLS);
  draw.text("SYSTEM STATUS", { x: panelWidth/2 - 18, y: ledY - 2, z: 0.2 }, 0.8);
...
function drawConnectorPanel(pen) {
  ...
  // Connector types
  const connectors = [
    { type: "power", x: -panelWidth/2 + 5, y: -3, width: 4, height: 4, label: "POWER" },
    { type: "serial", x: -panelWidth/2 + 14, y: -3, width: 6, height: 3, label: "SERIAL" },
    { type: "network", x: -panelWidth/2 + 24, y: -3, width: 3, height: 3, label: "NETWORK" },
    { type: "expansion", x: -panelWidth/2 + 32, y: -3, width: 6, height: 6, label: "EXPANSION" }
  ];
  ...
  for (const conn of connectors) {
    ...
    // Connector label
    applyMaterial(pen, MATERIALS.CONTROLS);
    draw.text(conn.label, { x: conn.x - conn.label.length * 0.3, y: conn.y - conn.height/2 - 1, z: 0.2 }, 0.7);
  }
  ...
  draw.text("CAUTION: HIGH VOLTAGE", { x: grillX - 8, y: grillY + grillHeight/2 + 1, z: 0.2 }, 0.7);
