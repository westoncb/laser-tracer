...
const K = RAW.map((v) => {
  ...
  return { kx: (v[0] / n) * KLEN, kz: (v[2] / n) * KLEN }; // slice ignores y-component
});

/* -------- 2 · sampling lattice (2-D slice) ------------------- */
const GRID = 50,
  STEP = 1; // 2,500 deposits / frame
const OFF = [];
...
function program(pen, scene, time) {
  ...
  if (firstFrame) {
    scene.setCamera({ x: 0, y: 0, z: 75 }, { x: 0, y: 0, z: 0 }); // Similar to default camera
    firstFrame = false;
  }

  // Setup pen appearance
  pen.dotSize(9).residue(RESID).fuzz(FUZZ_N, FUZZ_R);
