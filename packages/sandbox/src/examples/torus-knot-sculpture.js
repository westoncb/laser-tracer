...
function generateTorusKnotCurve(params, t, timeOffset = 0) {
  ...
  const wave =
    Math.sin(pAngle * 3 + timeOffset * 5) *
...
function generateCrossSection(center, frame, radius, segments, twist) {
  ...
  return circle.map((point) => {
    ...
    const twistedX =
      point.x * Math.cos(twistRad) - point.y * Math.sin(twistRad);
    const twistedY =
      point.x * Math.sin(twistRad) + point.y * Math.cos(twistRad);
...
function drawTorusKnot(pen, params, time) {
  ...
  for (let i = 0; i < radialSegments; i++) {
    ...
    for (let j = 0; j <= tubularSegments; j++) {
      ...
      const tubeRadius =
        thickness *
        (0.8 + Math.sin(t * Math.PI * params.p * 2 + time * 2) * 0.2);
  ...
  const ringInterval = Math.max(
    3,
    Math.floor(tubularSegments * (1 - detail) * 0.1),
  );
  for (let i = 0; i < tubularSegments; i += ringInterval) {
    ...
    const tubeRadius =
      thickness * (0.8 + Math.sin(t * Math.PI * params.p * 2 + time * 2) * 0.2);
    const twist = t * twistRate;
...
function drawEnergyFlows(pen, params, time) {
  ...
  pen
    .colorRGB(VISUAL.energyColor.r, VISUAL.energyColor.g, VISUAL.energyColor.b)
  ...
  for (let h = 0; h < harmonics; h++) {
    ...
    for (let i = 0; i <= spiralSteps; i++) {
      ...
      spiralPoints.push(generateEnergySpiral(params, t, h, timeOffset));
    }
  ...
  for (const node of nodes) {
    ...
    if (node.intensity > 0.7) {
      ...
      for (let i = 0; i < rayCount; i++) {
        ...
        // Direction vector
        const dir = rotatePointWithQuaternion(
          { x: rayLength, y: 0, z: 0 },
          quat,
        );
