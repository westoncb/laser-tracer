...
function drawMonitor(pen, highlight = false) {
  ...
  pen.polyline(
    [
      {
        x: -monitorWidth / 2 - bezelSize,
        y: -monitorHeight / 2 - bezelSize,
        z: 0,
      },
      {
        x: monitorWidth / 2 + bezelSize,
  ...
  pen.polyline(
    [
      ...
      {
        x: standWidth / 2,
        y: monitorHeight / 2 + bezelSize + standHeight,
        z: 0,
      },
      {
        x: -standWidth / 2,
  ...
  for (let i = 0; i < 3; i++) {
    ...
    pen.polyline(
      [
        ...
        { x: monitorWidth / 2 + bezelSize, y: y + 0.8, z: -monitorDepth / 4 },
        {
          x: monitorWidth / 2 + bezelSize + 1.5,
          y: y + 0.8,
          z: -monitorDepth / 4,
        },
        {
  ...
  pen.moveTo(
    monitorWidth / 2 + bezelSize - 1,
    monitorHeight / 2 + bezelSize - 1,
    0,
  );
...
function drawTerminalMode(pen, time, width, height) {
  ...
  // Terminal header
  scene.text(
    "VECTOR OS v3.14.15 - TERMINAL",
    { x: 0, y: height / 2 - 2, z: 0 },
    0.9,
  );
  ...
  for (let i = 0; i < visibleLines; i++) {
    scene.text(
      commands[i],
      { x: -width / 2 + 3, y: height / 2 - 5 - i * 1.5, z: 0 },
      0.7,
    );
  }
  ...
  if (visibleLines >= commands.length) {
    ...
    if (cursorBlink) {
      scene.text(
        "_",
        {
          x: -width / 2 + 3 + commands[commands.length - 1].length * 0.42,
          y: height / 2 - 5 - (commands.length - 1) * 1.5,
          z: 0,
        },
        0.7,
      );
  ...
  // Status bar
  scene.text(
    "READY | MEMORY: 64K | DISK: 1.2MB FREE",
    { x: 0, y: -height / 2 + 2, z: 0 },
    0.6,
  );
...
function drawBootSequence(pen, time, width, height) {
  ...
  for (let i = 0; i < messageCount; i++) {
    scene.text(messages[i], { x: 0, y: -8 + i * 3, z: 0 }, 0.8);
  }

  // Show percentage
  scene.text(
    "BOOT: " + Math.floor(bootProgress * 100) + "%",
    { x: 0, y: 5, z: 0 },
    1,
  );
...
function drawGridMode(pen, time, width, height) {
  ...
  for (let x = -width / 2 + gridSpacing; x < width / 2; x += gridSpacing) {
    scene.text(x.toString(), { x: x, y: -height / 2 + 1, z: 0 }, 0.6);
  }

  for (let y = -height / 2 + gridSpacing; y < height / 2; y += gridSpacing) {
    scene.text(y.toString(), { x: -width / 2 + 1, y: y, z: 0 }, 0.6);
  }
  ...
  // Draw system status
  scene.text("GRID MODE: READY", { x: 0, y: height / 2 - 2, z: 0 }, 1);
  scene.text(
    "PRESS F1 FOR HELP | F2 FOR CAD MODE",
    { x: 0, y: -height / 2 + 2, z: 0 },
    0.7,
  );
...
function drawCADMode(pen, time, width, height) {
  ...
  scene.text(
    "CAD MODEL: " + solidNames[solidType],
    { x: 0, y: height / 2 - 2, z: 0 },
    1,
  );
  ...
  scene.text("PROPERTIES:", { x: propX, y: height / 2 - 5, z: 0 }, 0.8);
  scene.text(
    "- VERTICES: " + getVertexCount(solidType),
    { x: propX + 2, y: height / 2 - 7, z: 0 },
    0.7,
  );
  scene.text(
    "- EDGES: " + getEdgeCount(solidType),
    { x: propX + 2, y: height / 2 - 9, z: 0 },
    0.7,
  );
  scene.text(
    "- FACES: " + getFaceCount(solidType),
    { x: propX + 2, y: height / 2 - 11, z: 0 },
    0.7,
  );

  // Dimensions
  scene.text("DIMENSIONS:", { x: propX, y: 0, z: 0 }, 0.8);
  scene.text("- SCALE: 7.00 UNITS", { x: propX + 2, y: -2, z: 0 }, 0.7);
  scene.text(
    "- ROTATION: " + Math.floor(screenRotation % 360) + "°",
    { x: propX + 2, y: -4, z: 0 },
    0.7,
  );
  ...
  scene.text("X", { x: -width / 2 + 11, y: -height / 2 + 6, z: 0 }, 0.7);
  ...
  scene.text("Y", { x: -width / 2 + 4, y: -height / 2 + 13, z: 0 }, 0.7);
  ...
  applyMaterial(pen, MATERIALS.SCREEN);
  scene.text(
    "VIEW CONTROLS | R-ROTATE | S-SCALE | TAB-NEXT MODEL",
    { x: 0, y: -height / 2 + 2, z: 0 },
    0.6,
  );
...
function drawKeyboard(pen, highlight = false) {
  ...
  const animatedCol = Math.floor(
    (keyboardAnimation * keyRows * keyCols) % keyCols,
  );
  ...
  if (!highlight) {
    ...
    scene.text(
      "SPACE",
      { x: 0, y: 2 + 4 * keySpacing, z: keyboardDepth * 1.3 },
      0.8,
    );
    ...
    for (let i = 0; i < 3; i++) {
      scene.text(
        "F" + (i + 1),
        {
          x: -keyboardWidth / 2 + 2 + (12 + i) * keySpacing,
          y: 2,
          z: keyboardDepth * 1.3,
        },
        0.6,
    ...
    for (let i = 0; i < 3; i++) {
      scene.text(
        arrowLabels[i],
        {
          x: -keyboardWidth / 2 + 2 + (11 + i) * keySpacing,
          y: 2 + 4 * keySpacing,
          z: keyboardDepth * 1.3,
        },
        0.6,
...
function drawSystemUnit(pen, highlight = false) {
  ...
  if (!highlight) {
    ...
    scene.text(
      '5.25" DRIVE',
      { x: unitWidth / 2, y: unitHeight / 2 - 4, z: 0.2 },
      0.6,
    );
    scene.text(
      '3.5" DRIVE',
      { x: unitWidth / 2, y: unitHeight / 2 - 8, z: 0.2 },
      0.6,
    );

    // Power button label
    scene.text("POWER", { x: 5, y: -unitHeight / 2 + 8, z: 0.2 }, 0.6);

    // System model label
    scene.text(
      "VECTOR-3000",
      { x: unitWidth / 2, y: -unitHeight / 2 + 15, z: 0.2 },
      0.9,
    );
    scene.text(
      "GRAPHICS WORKSTATION",
      { x: unitWidth / 2, y: -unitHeight / 2 + 13, z: 0.2 },
      0.6,
    );
  }
...
function drawSystemLabels(pen, time) {
  ...
  if (powerState) {
    ...
    if (focusedComponent === "monitor") {
      scene.text("VECTOR DISPLAY", { x: 0, y: -20, z: 15 }, 1.5);
      scene.text("1024x768 RESOLUTION", { x: 0, y: -23, z: 15 }, 1);
      scene.text("REFRESH RATE: 60Hz", { x: 0, y: -26, z: 15 }, 1);
    }

    // Keyboard label
    if (focusedComponent === "keyboard") {
      scene.text("ERGONOMIC KEYBOARD", { x: 0, y: 20, z: 15 }, 1.5);
      scene.text("101 KEYS", { x: 0, y: 17, z: 15 }, 1);
      scene.text("PROGRAMMABLE FUNCTION KEYS", { x: 0, y: 14, z: 15 }, 1);
    }

    // System unit label
    if (focusedComponent === "system") {
      scene.text("VECTOR-3000 CPU", { x: 25, y: 5, z: 15 }, 1.5);
      scene.text("32MHz PROCESSOR", { x: 25, y: 2, z: 15 }, 1);
      scene.text("64K VECTOR MEMORY", { x: 25, y: -1, z: 15 }, 1);
      scene.text('DUAL 5.25" / 3.5" DRIVES', { x: 25, y: -4, z: 15 }, 1);
    }

    // Main system label when no component is focused
    if (!focusedComponent) {
      scene.text("VECTOR-3000 GRAPHICS WORKSTATION", { x: 0, y: -35, z: 0 }, 2);
      ...
      for (let i = 0; i < specs.length; i++) {
        scene.text(specs[i], { x: 0, y: -40 - i * 3, z: 0 }, 1.2);
      }
    }

    // Show time and power info
    scene.text(
      "SYSTEM TIME: " + Math.floor(time).toString().padStart(5, "0") + "s",
      { x: -30, y: -35, z: 0 },
      1,
    );
    scene.text(
...
function program(pen, scene, time) {
  ...
  if (frameCount === 0) {
    ...
    // Set camera position
    scene.setCamera({ x: 0, y: 0, z: 85 }, { x: 0, y: 0, z: 0 });
