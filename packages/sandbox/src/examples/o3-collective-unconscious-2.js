...
const CORE_R = 10;
const RINGS = [
  { r: 9, n: 6, speed: 6, hue: 0.0 },
  { r: 13, n: 10, speed: -4, hue: 0.1 },
  { r: 17, n: 14, speed: 3, hue: 0.25 },
  { r: 21, n: 18, speed: -2, hue: 0.4 },
];
...
function nodePos(rSpec, k, time) {
  const ang =
    (k / rSpec.n) * Math.PI * 2 + (time * rSpec.speed * Math.PI) / 180;
  return {
...
function program(pen, scene, time) {
  if (first) {
    scene.setBGColor(0x000007);
    for (let i = 0; i < SWARM_N; i++) spawnParticle(i, 0);
    first = false;
    scene.setCamera({ x: 0, y: 0, z: 60 }, { x: 0, y: 0, z: 0 });
  }
  ...
  pen.dotSize(3).fuzz(3, 0.2).residue(2);
  for (let s = 0; s < 18; s++) {
